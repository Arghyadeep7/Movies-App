...
const MovieComponent = (props) => {
  ...
  const type = props.type==="trending"?"trending/movie/day":"movie/"+props.type;
  ...
  const fetchMovies=async ()=>{
    try{
      ...
      const movieData=await axios.get(`https://api.themoviedb.org/3/${type}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=${count}`)
        .then(res => res.data);
